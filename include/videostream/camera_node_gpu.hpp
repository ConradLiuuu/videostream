...
public:
  CameraNodeGPU(const std::string& name, const unsigned int& serial_number){
    ...
    pub_center = nh.advertise<std_msgs::Int64MultiArray>(tmp_string, 1, false);
    ...
    cout << baseName << " camera setting frameRate = " << frmRate_get.absValue << endl;
  ...
  ~CameraNodeGPU(){
    cout << "Closing " << baseName << " camera" << endl;
  ...
  void ImageProcessing(const std_msgs::Bool::ConstPtr& msg){
    ROS_INFO_STREAM(baseName << " camera starts to do image process" << cnt_proc);

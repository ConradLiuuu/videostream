...
{
  ROS_INFO("Right camera called function");
}

int main(int argc, char **argv)
{
  ros::init(argc, argv, "image_process_right");
  ros::NodeHandle nh;
  ros::Publisher pub_frameRate = nh.advertise<std_msgs::Float32>("frameRate_right",1,false);
  ros::Publisher pub_center = nh.advertise<std_msgs::Float64MultiArray>("ball_center_right", 1, false);
  ros::Subscriber sub;
  ...
  image_transport::Publisher pub_binary = it.advertise("right_camera_binary",1);
  ...
  cv::Mat img, img_hsv, img_binary, element;
  Image rawImage;
  Image bgrImage;
  ...
  ROS_INFO("Right camera setting frameRate = %f", frmRate.absValue);
  ...
  // Capture loop
  ROS_INFO("Start to do right camera image process");
  while (ros::ok()){
    ...
    */
    ...
    //ROS_INFO("Right camera start to do image process % d", cnt);

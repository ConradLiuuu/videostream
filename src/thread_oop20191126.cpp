...
public:
  ...
  Camera_L(){
    ...
    timer = nh.createTimer(ros::Duration(1.0 / 100),std::bind(&Camera_L::capture, this));
  }
  ...
  void operator()(){
    //timer = nh.createTimer(ros::Duration(1.0 / 100),std::bind(&Camera_L::capture, this));
    sub = nh.subscribe("/sampling_time", 1, &Camera_L::callback, this);
  }
  ...
  void callback(const std_msgs::Bool::ConstPtr& msg){
    ...
    if ((center.x == 0) && (center.y == 0)){
      //cout << "not find ball \n";
      img_serve = img2(cv::Rect(T_one2ori.x, T_one2ori.y, 640, 240));
      delta = cv::Point2f(0,0);
      center_in_world_frame = cv::Point2f(-1,-1);
      center_L = cv::Point2f(-1,-1);
...
public:
  ...
  Camera_R(){
    ...
    // start capture image
    error = camera.StartCapture();
    timer = nh.createTimer(ros::Duration(1.0 / 100),std::bind(&Camera_R::capture, this));
  }
  ...
  void operator()(){
    //timer = nh.createTimer(ros::Duration(1.0 / 100),std::bind(&Camera_R::capture, this));
    sub = nh.subscribe("/sampling_time", 1, &Camera_R::callback, this);
  }
...
void stereo(){
  cout << "center left = " << center_L << endl;
}
